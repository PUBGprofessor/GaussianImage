...
class SimpleTrainer2d:
    ...
    def __init__(
        ...
        self.log_dir = Path(f"./checkpoints/{model_name}_{args.iterations}_{num_points}/{self.image_name}")
...
def image_path_to_tensor(image_path: Path):
    img = Image.open(image_path).convert("RGB")  # 转换为 RGB
...
def main(argv):
    ...
    logwriter = LogWriter(Path(f"./checkpoints/{args.model_name}_{args.iterations}_{args.num_points}"))
